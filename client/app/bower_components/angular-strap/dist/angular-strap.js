...
(function(window, document, undefined) {
  'use strict';
  angular.module('mgcrea.ngStrap', [ 'mgcrea.ngStrap.modal', 'mgcrea.ngStrap.aside', 'mgcrea.ngStrap.alert', 'mgcrea.ngStrap.button', 'mgcrea.ngStrap.select', 'mgcrea.ngStrap.datepicker', 'mgcrea.ngStrap.timepicker', 'mgcrea.ngStrap.navbar', 'mgcrea.ngStrap.bstooltip', 'mgcrea.ngStrap.popover', 'mgcrea.ngStrap.dropdown', 'mgcrea.ngStrap.typeahead', 'mgcrea.ngStrap.scrollspy', 'mgcrea.ngStrap.affix', 'mgcrea.ngStrap.tab', 'mgcrea.ngStrap.collapse' ]);
  ...
  angular.module('mgcrea.ngStrap.datepicker', [ 'mgcrea.ngStrap.helpers.dateParser', 'mgcrea.ngStrap.helpers.dateFormatter', 'mgcrea.ngStrap.bstooltip' ]).provider('$datepicker', function() {
    var defaults = this.defaults = {
      animation: 'am-fade',
  ...
  angular.module('mgcrea.ngStrap.dropdown', [ 'mgcrea.ngStrap.bstooltip' ]).provider('$dropdown', function() {
    var defaults = this.defaults = {
      animation: 'am-fade',
  ...
  angular.module('mgcrea.ngStrap.popover', [ 'mgcrea.ngStrap.bstooltip' ]).provider('$popover', function() {
    var defaults = this.defaults = {
      animation: 'am-fade',
      customClass: '',
      container: false,
  ...
  angular.module('mgcrea.ngStrap.select', [ 'mgcrea.ngStrap.bstooltip', 'mgcrea.ngStrap.helpers.parseOptions' ]).provider('$select', function() {
    var defaults = this.defaults = {
      animation: 'am-fade',
      prefixClass: 'select',
      prefixEvent: '$select',
  ...
  angular.module('mgcrea.ngStrap.timepicker', [ 'mgcrea.ngStrap.helpers.dateParser', 'mgcrea.ngStrap.helpers.dateFormatter', 'mgcrea.ngStrap.bstooltip' ]).provider('$timepicker', function() {
    var defaults = this.defaults = {
      animation: 'am-fade',
  ...
  angular.module('mgcrea.ngStrap.typeahead', [ 'mgcrea.ngStrap.bstooltip', 'mgcrea.ngStrap.helpers.parseOptions' ]).provider('$typeahead', function() {
    var defaults = this.defaults = {
      animation: 'am-fade',
  ...
  angular.module('mgcrea.ngStrap.bstooltip', [ 'mgcrea.ngStrap.core', 'mgcrea.ngStrap.helpers.dimensions' ]).provider('$bstooltip', function() {
    var defaults = this.defaults = {
      animation: 'am-fade',
      customClass: '',
      prefixClass: 'bstooltip',
      prefixEvent: 'bstooltip',
      container: false,
      target: false,
      placement: 'top',
      templateUrl: 'bstooltip/bstooltip.tpl.html',
      template: '',
      contentTemplate: false,
      trigger: 'hover focus',
      keyboard: false,
    ...
    this.$get = [ '$window', '$rootScope', '$bsCompiler', '$q', '$templateCache', '$http', '$animate', '$sce', 'dimensions', '$$rAF', '$timeout', function($window, $rootScope, $bsCompiler, $q, $templateCache, $http, $animate, $sce, dimensions, $$rAF, $timeout) {
      ...
      function bstooltipFactory(element, config) {
        var $bstooltip = {};
        ...
        function replaceArrow(delta, dimension, isHorizontal) {
          var $arrow = findElement('.bstooltip-arrow, .arrow', tipElement[0]);
      ...
      return bstooltipFactory;
    } ];
  }).directive('bsbstooltip', [ '$window', '$location', '$sce', '$bstooltip', '$$rAF', function($window, $location, $sce, $bstooltip, $$rAF) {
    return {
      restrict: 'EAC',
      scope: true,
      link: function postLink(scope, element, attr, transclusion) {
        ...
        attr.$observe('title', function(newValue) {
          if (angular.isDefined(newValue) || !scope.hasOwnProperty('title')) {
            ...
            angular.isDefined(oldValue) && $$rAF(function() {
              bstooltip && bstooltip.$applyPlacement();
            });
          }
        });
        attr.bsbstooltip && scope.$watch(attr.bsbstooltip, function(newValue, oldValue) {
          if (angular.isObject(newValue)) {
            angular.extend(scope, newValue);
          } else {
        ...
        attr.bsShow && scope.$watch(attr.bsShow, function(newValue, oldValue) {
          if (!bstooltip || !angular.isDefined(newValue)) return;
          if (angular.isString(newValue)) newValue = !!newValue.match(/true|,?(bstooltip),?/i);
          newValue === true ? bstooltip.show() : bstooltip.hide();
        });
        attr.bsEnabled && scope.$watch(attr.bsEnabled, function(newValue, oldValue) {
          if (!bstooltip || !angular.isDefined(newValue)) return;
          if (angular.isString(newValue)) newValue = !!newValue.match(/true|1|,?(bstooltip),?/i);
          newValue === false ? bstooltip.setEnabled(false) : bstooltip.setEnabled(true);
        });
        attr.viewport && scope.$watch(attr.viewport, function(newValue) {
          if (!bstooltip || !angular.isDefined(newValue)) return;
          bstooltip.setViewport(newValue);
        });
        var bstooltip = $bstooltip(element, options);
        scope.$on('$destroy', function() {
          if (bstooltip) bstooltip.destroy();
          options = null;
          bstooltip = null;
        });
      }
