...
def config_routes(config):
    ...
    config.add_route('myfiles','/myfiles',
                 factory='spinaltoolbox.security.SecurityFactory')
...
def config_auth_policy(config, settings):
    policy = authentication.AuthTktAuthenticationPolicy(settings['auth_secret'], get_principals, cookie_name="spinaltoolbox_auth", hashalg="sha512")
